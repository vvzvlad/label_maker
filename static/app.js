    ...
    function showContextMenu(x, y, node) {
      ctxTarget = node;
      ctxIsMultiSelect = transformer.nodes().length >= 2;
    ...
    ctxDelete.addEventListener('click', () => {
      const nodesToDelete = transformer.nodes().length >= 2
        ? transformer.nodes().slice()
        : (ctxTarget ? [ctxTarget] : []);
    ...
    ctxDuplicate.addEventListener('click', () => {
      const nodesToDuplicate = transformer.nodes().length >= 2
        ? transformer.nodes().slice()
        : (ctxTarget ? [ctxTarget] : []);
    ...
    function alignSelectedNodes(type) {
      ...
      const rects = nodes.map(node => node.getClientRect({ relativeTo: layer }));
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Delete' || e.key === 'Backspace') {
        const nodesToDelete = transformer.nodes().slice();
        if (nodesToDelete.length > 0) {
          e.preventDefault();
          nodesToDelete.forEach((node) => node.destroy());
          transformer.nodes([]);
     ...
     */
    function selectWithTransformer(nodes, anchors = null, addToSelection = false) {
      ...
      if (addToSelection) {
        ...
        if (updatedNodes.length > 1) {
          transformer.enabledAnchors(fullSelectionAnchors);
    ...
    function initStage(preserveNodes = true) {
      ...
      // Deselect when clicking on the gray area outside the label canvas
    ...
    function pushHistory() {
      ...
      historyStack.push(serializeLayerNodes());
    ...
    function captureCurrentState() {
      ...
      return {
        ...
        table: { columnCount: entityColumnCount, rows },
      };
    }
    ...
    function applyPresetState(preset) {
      ...
      if (tbl) {
        const tbody = document.getElementById('entities-tbody');
        tbody.innerHTML = '';
        entityColumnCount = Number.isInteger(tbl.columnCount) && tbl.columnCount > 0
          ? tbl.columnCount
          : 3;
        renderTableHeader();
